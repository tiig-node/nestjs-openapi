...
describe('RedocDocumentModel', () => {
  ...
  describe('X-tagGroups', () => {
    ...
    describe('When tagGroups is not provided or empty', () => {
      const expected = [
        { name: 'user', tags: ['user'] },
        { name: 'customer', tags: ['client'] },
      ];
      const document = {
        tags: [{ name: 'user' }, { name: 'client', description: 'customer' }],
      } as any;
  ...
  describe('X-codeSamples', () => {
    ...
    beforeEach(() => {
      ...
      document = {
        paths: {
          '/endpoint/test1': { get: {} },
          '/endpoint/test2': {
            post: { 'x-codeSamples': [{ lang: 'js', source: 'same code' }] },
          },
        },
      } as any;
    });
    ...
    describe('When skipSnippetsGeneration is disabled', () => {
      it('Should call code generation for not supplied x-codeSamples', () => {
        ...
        const redocDoc = RedocDocumentModel.fromOpenApi(document, {
          codeSnippetsLanguages: ['javascript_xhr'],
        });
        expect(getEndpointSnippetsSpy).toHaveBeenNthCalledWith(
          1,
          redocDoc,
          '/endpoint/test1',
          'get',
        ...
        expect(
          redocDoc.paths['/endpoint/test1']['get']['x-codeSamples'],
        ).toEqual([{ lang: 'title', source: 'content' }]);
        expect(
          redocDoc.paths['/endpoint/test2']['post']['x-codeSamples'],
        ).toEqual([{ lang: 'js', source: 'same code' }]);
      });

      it('Should throw SnippetGenerateError', () => {
        getEndpointSnippetsSpy.mockImplementation(() => {
          throw new Error();
        });
        expect(() => RedocDocumentModel.fromOpenApi(document)).toThrow(
          SnippetGenerateError,
        );
      });
    });
  });
