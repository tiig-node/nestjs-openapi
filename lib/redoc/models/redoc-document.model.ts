...
import {
  RedocDocument,
  CodeSampleObject,
} from '../interfaces/redoc-document.interface';
import {
  RedocModuleOptions,
  TagGroupOptions,
} from '../interfaces/redoc-module-options.interface';
...
export class RedocDocumentModel {
  ...
  private static createTagsGroup(document: RedocDocument): TagGroupOptions[] {
    return (document as any).tags?.map((tag) => ({
  ...
  private static addTagsGroup(
    ...
    if (options?.tagGroups || (document as any).tags) {
      document['x-tagGroups'] = options?.tagGroups?.length
        ? options.tagGroups
        : RedocDocumentModel.createTagsGroup(document);
    }
  }
  ...
  private static generateCodeSample(
    ...
    const endpoint = getEndpointSnippets(
      document as OpenAPIObject,
      path,
      method,
      languages,
    );
    return endpoint.snippets.map((snippet) => ({
      lang: snippet.title,
      source: normalizeSnippetCode(snippet.content),
    }));
  }
  ...
  private static addCodeSamples(
    ...
    const codeSnippetsLanguages = options?.codeSnippetsLanguages || [
      'javascript',
    ];
      ...
      for (const method in operations) {
        ...
        try {
          operation['x-codeSamples'] =
            operation['x-codeSamples'] ||
            RedocDocumentModel.generateCodeSample(
              document,
              path,
              method as HttpMethod,
              codeSnippetsLanguages,
            );
        } catch (e) {
