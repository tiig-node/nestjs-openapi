...
describe('RedocTryOutModule', () => {
  ...
  describe('Setup', () => {
    ...
    const document: OpenAPIObject = {
      info: { title: 'title', version: '2' },
      openapi: '',
      paths: {},
    };
    const redoDocument: RedocDocument = {
      info: { title: 'title', version: '2' },
      openapi: '',
      paths: {},
      'x-tagGroups': [],
    ...
    describe('RedocDocument', () => {
      it('Should convert document to redocDocument', async () => {
        ...
        expect(RedocDocumentModel.fromOpenApi).toBeCalledWith(
          document,
          undefined,
        );
      });
      ...
      it('Should throw RedocTryOutModuleError', () => {
        RedocDocumentModel.fromOpenApi = jest.fn().mockImplementation(() => {
          throw new Error();
        });
        expect(() =>
          RedocTryOutModule.setup('doc/api', app, document, options),
        ).rejects.toThrow(RedocTryOutModuleError);
      });
    });

    describe('Initialize adapter handler', () => {
      describe('Path normalize', () => {
        const expectPathTest = async (
          path: string,
          expectedPath: string,
        ): Promise<void> => {
          await RedocTryOutModule.setup(path, app, document);
          expect(AdapterHandler.init).toBeCalledWith(
            expect.anything(),
            expectedPath,
            expect.anything(),
            undefined,
      ...
      it('Should throw RedocTryOutModuleError', () => {
        AdapterHandler.init = jest.fn().mockImplementation(() => {
          throw new Error();
        });
        expect(() =>
          RedocTryOutModule.setup('doc/api', app, document, options),
