...
abstract class ParameterItem<
  Scheme extends ParameterObject | SecuritySchemeObject,
> implements IHarItem
{
...
class Parameters implements IParameters {
  ...
  constructor(parameters: ParameterObject[], parent?: Parameters) {
    this.parameters = (parameters || []).map(
      (param) => new ParameterToHar(param),
    );
    this.parent = parent;
  }

  private merge(
    parameters: ParameterToHar[],
    parentParameters: ParameterToHar[],
  ): IHarItem[] {
    (parentParameters || []).forEach((parentParameter) => {
      if (
        !parameters.find((parameter) => parameter.isEqualTo(parentParameter))
      ) {
...
class MethodSecurityRequirements {
  ...
  protected get nonPublicRequirements(): string[] {
    return this.securityInfo.requirements.filter(
...
class PostDataToHar {
  ...
  private encodeURIParam(param: { name: string; value: string }): {
    name: string;
    value: string;
  } {
  ...
  private sampleToFormUrlEncoded(
    ...
    if (type === 'application/x-www-form-urlencoded') {
      const params = Object.entries(sample).map(([name, value]) =>
        this.encodeURIParam({ name, value }),
      );
      return {
        mimeType: type,
        params: params,
  ...
  private get availableContentTypes(): string[] {
    return Object.keys(this.content).filter((contentType: MimeType) =>
      VALID_MIME_TYPES.includes(contentType),
    );
  }
  ...
  private get availableContents(): {
    ...
    return this.availableContentTypes
      .map((contentType) => ({
        mimeType: contentType as MimeType,
        schema: this.content[contentType].schema as SchemaObject,
      }))
...
class MethodToHar implements IMethodToHar {
  ...
  constructor(scheme: OperationObjectWrapper, parent: PathToHar) {
    const parameters = new Parameters(
      scheme.parameters as ParameterObject[],
      parent.parameters,
    );
    ...
    this.headers = [
      ...parameters.getHeaderParameters(),
  ...
  private metaData(postData?: IHarPostData): {
    headers: IHarItem[];
    comment: string;
  } {
    const comment = postData?.mimeType || '';
    const headers = postData?.mimeType
      ? [...this.headers, { name: 'content-type', value: postData.mimeType }]
      : this.headers;
  ...
  private methodAsArray(): IHarMethod[] {
    this.methodArray = this.postData.isEmpty
      ? [this.methodToHar()]
      : this.postData.toArray().map((postData) => this.methodToHar(postData));
...
export class ApiToHar {
  ...
  private pathsAsArray(): IHarPath[] {
    this.pathsArray = [];
    this.pathsArray = this.pathsArray.concat(
      ...this.paths.map((path) => path.toArray()),
    );
